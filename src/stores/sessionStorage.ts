...
class SessionStorageManager {
  ...
  async load(projectId: string): Promise<PyxisSession> {
    const session = await storageService.get<PyxisSession>(STORES.SESSION_STATE, this.getSessionKey(projectId));
  ...
  async loadUIState(projectId: string): Promise<PyxisSession['ui']> {
    const saved = await storageService.get<PyxisSession['ui']>(STORES.SESSION_STATE, this.getUIKey(projectId));
