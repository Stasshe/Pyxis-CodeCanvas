  ...
  async execute(args: string[] = []): Promise<string> {
    ...
    if (decompress) {
      ...
      try {
        ...
        if (this.terminalUI)
          await this.terminalUI.spinner.error(
            `Decompression failed: ${err?.message || String(err)}`
          );
        throw err;
      }
    } else {
      ...
      try {
        const contentArray = target.bufferContent
          ? new Uint8Array(target.bufferContent)
