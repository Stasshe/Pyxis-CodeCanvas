...
export class DfCommand extends UnixCommandBase {
  async execute(_: string[] = []): Promise<string> {
    ...
    const used = all.reduce(
      (s, f) => s + (f.bufferContent?.byteLength || f.content?.length || 0),
      0
    );
    ...
    lines.push(
      `pyxis:${this.projectName}${toK(TOTAL_BYTES)}${toK(used)}${toK(avail)} ${(Math.round((used / TOTAL_BYTES) * 100) || 0).toString().padStart(4)}% /`
    );
    return lines.join('\n');
  }
}
